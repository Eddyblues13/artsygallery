...
<div class="row">
    <div class="col-lg-8 mx-auto">
        <form action="{{ route('admin.nft-drops.update', $nftDrop->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="mb-3">
                ...
                <input type="number" step="0.01" class="form-control @error('eth_value') is-invalid @enderror"
                    id="eth_value" name="eth_value" value="{{ old('eth_value', $nftDrop->eth_value) }}" required>
                @error('eth_value')
                <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
