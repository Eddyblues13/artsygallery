					...
					<div class="card-body text-center p-5">
						<div class="avatar-lg mx-auto mb-3 position-relative">
							...
							<div class="rounded-circle overflow-hidden shadow-lg"
								style="width: 100px; height: 100px; margin: 0 auto; border: 4px solid #fff;">
								<img src="{{ $userProfile->profile_picture }}" alt="{{ $userProfile->name }}"
							...
							<div class="rounded-circle d-flex align-items-center justify-content-center shadow-lg"
								...
								<span class="text-white display-5 fw-bold">{{ strtoupper(substr($userProfile->name, 0,
									1)) }}</span>
							</div>
							@endif
							<label for="adminProfilePicInput"
								class="position-absolute bg-primary text-white rounded-circle d-flex align-items-center justify-content-center"
								style="width: 32px; height: 32px; cursor: pointer; border: 3px solid #fff; bottom: 0; left: 50%; transform: translateX(20px);"
								title="Change Profile Picture">
								<i data-feather="camera" style="width: 14px; height: 14px;"></i>
							</label>
							<span
								class="position-absolute bottom-0 start-50 translate-middle-x badge rounded-pill bg-{{ $userProfile->is_activated == '1' ? 'success' : 'warning' }} border border-white">
								{{ $userProfile->is_activated == '1' ? 'Active' : 'Inactive' }}
							</span>
						</div>
						<form action="{{ route('admin.upload.profile.picture', $userProfile->id) }}" method="POST"
							enctype="multipart/form-data" id="adminProfilePicForm">
							@csrf
							<input type="file" name="profile_picture" id="adminProfilePicInput" accept="image/*"
								class="d-none" onchange="document.getElementById('adminProfilePicForm').submit();">
						</form>
						...
						<div class="d-grid gap-2">
							<form action="{{ route('toggle.wallet.verify', $userProfile->id) }}" method="POST">
								...
								@method('PUT')
								<button type="submit"
									class="btn btn-{{ $userProfile->wallet_verify ? 'outline-success' : 'outline-danger' }} w-100 btn-lg rounded-pill">
							...
							<div class="row g-2">
								...
								<div class="col-6">
									...
									<a href="{{ route('use_linking_withdrawal', $userProfile->id) }}"
										class="btn btn-light w-100" title="Switch to Linked Withdrawals">
										<i class="align-middle" data-feather="link"></i> Linking
									</a>
									@else
									<a href="{{ route('none_linking_withdrawal', $userProfile->id) }}"
										class="btn btn-light w-100" title="Switch to Direct Withdrawals">
					...
					<div class="card-footer bg-light border-0 p-4">
						...
						<div class="vstack gap-3">
							<div class="d-flex justify-content-between">
								<span class="text-muted"><i class="align-middle me-2" width="16"
										data-feather="smartphone"></i> Phone</span>
								<span class="fw-medium">{{ $userProfile->phone ?? 'N/A' }}</span>
							</div>
							<div class="d-flex justify-content-between">
								<span class="text-muted"><i class="align-middle me-2" width="16"
										data-feather="calendar"></i> Joined</span>
								<span class="fw-medium">{{ \Carbon\Carbon::parse($userProfile->created_at)->format('M d,
									Y') }}</span>
							</div>
							<div class="d-flex justify-content-between">
								<span class="text-muted"><i class="align-middle me-2" width="16"
										data-feather="map-pin"></i> Country</span>
						...
						<div class="row g-2">
							<div class="col-auto">
								<button type="button" class="btn btn-success btn-sm rounded-pill px-3"
									data-bs-toggle="modal" data-bs-target="#addProfitModal">
							...
							<div class="col-auto">
								<button type="button" class="btn btn-danger btn-sm rounded-pill px-3"
									data-bs-toggle="modal" data-bs-target="#debitProfitModal">
							...
							<div class="col-auto">
								<button type="button" class="btn btn-primary btn-sm rounded-pill px-3"
									data-bs-toggle="modal" data-bs-target="#activationFeeModal">
									<i class="align-middle me-1" data-feather="dollar-sign"></i> Activation Fee
								</button>
							</div>
							<div class="col-auto ms-auto">
								<div class="dropdown">
									<button class="btn btn-light btn-sm rounded-pill px-3 dropdown-toggle" type="button"
									...
									<ul class="dropdown-menu dropdown-menu-end shadow-sm border-0"
										...
										<li>
											<h6 class="dropdown-header">NFT Management</h6>
										</li>
										<li><a class="dropdown-item"
												href="{{ url('user_approved_nft/' . $userProfile->id) }}">Approved
												Artworks</a></li>
										<li><a class="dropdown-item"
												href="{{ url('user_unapproved_nft/' . $userProfile->id) }}">Pending
												Artworks</a></li>
										<li><a class="dropdown-item"
												href="{{ url('user_sold_nft/' . $userProfile->id) }}">Sold Artworks</a>
										</li>
										<li>
						...
						<ul class="nav nav-tabs card-header-tabs" id="userTabs" role="tablist">
							<li class="nav-item" role="presentation">
								<button class="nav-link active" id="overview-tab" data-bs-toggle="tab"
									data-bs-target="#overview" type="button" role="tab">Overview & KYC</button>
							</li>
							<li class="nav-item" role="presentation">
								<button class="nav-link" id="methods-tab" data-bs-toggle="tab" data-bs-target="#methods"
									type="button" role="tab">Linked Methods</button>
							</li>
							<li class="nav-item" role="presentation">
								<button class="nav-link" id="deposits-tab" data-bs-toggle="tab"
									data-bs-target="#deposits" type="button" role="tab">Deposits</button>
							</li>
							<li class="nav-item" role="presentation">
								<button class="nav-link" id="withdrawals-tab" data-bs-toggle="tab"
									data-bs-target="#withdrawals" type="button" role="tab">Withdrawals</button>
							</li>
						</ul>
					</div>
						...
						<div class="tab-content">
							...
							<div class="tab-pane fade show active p-4" id="overview" role="tabpanel">
								...
								<div class="row g-4 mb-5">
									<div class="col-md-6">
										<h5 class="fw-bold mb-3"><i class="align-middle me-2 text-primary"
												data-feather="credit-card"></i> Wallet Information</h5>
										@if($userProfile->wallet_linked)
										<div class="card bg-light border-0">
											<div class="card-body">
												<div class="mb-2">
													<span class="text-muted d-block small text-uppercase fw-bold">Wallet
														Type</span>
													<span class="fs-5 fw-bold">{{ ucfirst($userProfile->wallet_type)
														}}</span>
												</div>
												<div class="mb-3">
													<span class="text-muted d-block small text-uppercase fw-bold">Wallet
														Address</span>
													<code
														class="text-break bg-white px-2 py-1 rounded border d-block mt-1">{{ $userProfile->wallet_address }}</code>
												</div>
												@if($userProfile->wallet_phrase)
												<div class="d-grid">
													<button type="button" class="btn btn-outline-primary btn-sm"
														data-bs-toggle="modal" data-bs-target="#walletPhraseModal">
														<i class="align-middle me-1" data-feather="key"></i> View
														Recovery Phrase
													</button>
												</div>
												@endif
									...
									<div class="col-md-6">
										<h5 class="fw-bold mb-3"><i class="align-middle me-2 text-primary"
												data-feather="file-check"></i> KYC Document</h5>
										@if($filePath)
										<div class="card border-0 shadow-sm text-center bg-light">
											<div class="card-body">
												...
												<div class="ratio ratio-16x9 mb-3">
													<img src="{{ $filePath }}"
														class="rounded shadow-sm object-fit-cover" alt="ID Card"
												...
												<div class="mb-3" style="height: 150px;">
													<div
														class="h-100 bg-white rounded border d-flex align-items-center justify-content-center">
														<i class="text-danger" data-feather="file-text" width="48"
															height="48"></i>
												...
												<div class="d-flex gap-2 justify-content-center">
													...
													<a href="{{ url('approve-id_card/' . $userProfile->id) }}"
														class="btn btn-sm btn-success"
														onclick="event.preventDefault(); document.getElementById('approve-kyc-form').submit();">
														<i class="align-middle" data-feather="check"></i>
													</a>
													<a href="{{ url('reject-id_card/' . $userProfile->id) }}"
														class="btn btn-sm btn-danger"
														onclick="event.preventDefault(); document.getElementById('reject-kyc-form').submit();">
														<i class="align-middle" data-feather="x"></i>
													</a>

													<form id="approve-kyc-form"
														action="{{ url('approve-id_card/' . $userProfile->id) }}"
														method="POST" class="d-none">
														@csrf <input type="hidden" name="status" value="1"> <input
															type="hidden" name="email"
															value="{{ $userProfile->email }}"> <input type="hidden"
															name="name" value="{{ $userProfile->name }}">
													</form>
													<form id="reject-kyc-form"
														action="{{ url('reject-id_card/' . $userProfile->id) }}"
														method="POST" class="d-none">
										...
										<div class="alert alert-light border text-center py-5">
											<i class="align-middle text-muted mb-2" data-feather="upload-cloud"
							...
							<div class="tab-pane fade p-4" id="methods" role="tabpanel">
									...
									<div class="col-md-6 mb-4">
										<div class="card h-100 border shadow-sm method-card">
											<div
												...
												<div class="d-flex align-items-center">
													<div class="icon-box rounded-circle bg-light d-flex align-items-center justify-content-center me-3"
													...
													<h5 class="fw-bold mb-0 text-capitalize">{{ $method->method_type }}
													</h5>
												</div>
												<span
													class="badge bg-light text-muted border rounded-pill px-3">Linked</span>
											</div>
											<div class="card-body px-4 pt-3 pb-4">
												@if($method->method_type == 'bank')
												<div class="mb-2"><span class="text-muted small">Bank Name:</span> <span
														class="fw-medium text-dark">{{ $method->bank_name }}</span>
												</div>
												...
												<div><span class="text-muted small">Routing No:</span> <code
														class="text-dark bg-light px-2 rounded">{{ $method->bank_routing_number }}</code>
												</div>
												@elseif($method->method_type == 'crypto')
												<div class="mb-2"><span class="text-muted small">Network:</span> <span
														class="badge bg-soft-success text-success">{{
														$method->crypto_type }}</span></div>
												<div><span class="text-muted small d-block mb-1">Wallet Address:</span>
													<code
														class="text-dark bg-light px-2 rounded d-block text-truncate">{{ $method->crypto_wallet_address }}</code>
												</div>
												@elseif($method->method_type == 'paypal')
												<div><span class="text-muted small">Email:</span> <a
														href="mailto:{{ $method->paypal_email }}" class="fw-medium">{{
														$method->paypal_email }}</a></div>
												@else
												<div><span class="text-muted small d-block mb-1">Details:</span> <span
														class="fw-medium">{{ $method->withdrawal_details }}</span></div>
												@endif
											</div>
											<div class="card-footer bg-light border-0 px-4 py-3">
												<small class="text-muted">Added {{ $method->created_at->diffForHumans()
													}}</small>
											</div>
										</div>
									</div>
								...
								<div class="text-center py-5">
									<div class="mb-3">
										<i class="text-muted" data-feather="credit-card" width="48" height="48"
											style="opacity: 0.2"></i>
									</div>
										...
										<tbody>
											...
											<tr>
												...
												<td class="px-4">
													...
													<span
														class="badge bg-soft-warning text-warning rounded-pill px-3">Pending</span>
													@elseif($depositHistory->status == '1')
													<span
														class="badge bg-soft-success text-success rounded-pill px-3">Approved</span>
													@elseif($depositHistory->status == '2')
													<span
														class="badge bg-soft-danger text-danger rounded-pill px-3">Declined</span>
													@endif
												</td>
												<td class="px-4 text-muted small">{{
													\Carbon\Carbon::parse($depositHistory->created_at)->format('M d, Y
													H:i A') }}</td>
												<td class="px-4 text-end">
													@if($depositHistory->status == '0')
													<div class="btn-group">
														<a href="{{ url('approve-deposit/' . $depositHistory->id) }}"
															class="btn btn-sm btn-success"
														...
														<a href="{{ url('decline-deposit/' . $depositHistory->id) }}"
															class="btn btn-sm btn-danger"
													...
													<form id="approve-dep-{{$depositHistory->id}}"
														action="{{ url('approve-deposit/' . $depositHistory->id) }}"
														method="POST" class="d-none">@csrf <input type="hidden"
															name="status" value="1"></form>
													<form id="decline-dep-{{$depositHistory->id}}"
														action="{{ url('decline-deposit/' . $depositHistory->id) }}"
														method="POST" class="d-none">@csrf <input type="hidden"
															name="status" value="2"></form>
													@else
											...
											@empty
											<tr>
												<td colspan="5" class="text-center py-4 text-muted">No records found
												</td>
											</tr>
											@endforelse
										</tbody>
								...
								<div class="table-responsive">
									<table class="table table-hover table-striped mb-0 align-middle">
										...
										<tbody>
											...
											<tr>
												...
												<td class="px-4">
													...
													<span class="badge bg-light text-dark border fw-normal">
														...
														$withdrawalHistory->crypto_type }}) @endif
													</span>
													@else - @endif
												</td>
												<td class="px-4">
												...
												<td class="px-4 text-muted small">{{
													\Carbon\Carbon::parse($withdrawalHistory->created_at)->format('M d,
													Y H:i A') }}</td>
												<td class="px-4 text-end">
													<div class="btn-group">
														@if($withdrawalHistory->withdrawal_method)
														...
														<a href="#" class="btn btn-sm btn-success"
															onclick="event.preventDefault(); if(confirm('Approve?')) document.getElementById('approve-wd-{{$withdrawalHistory->id}}').submit();">
															<i data-feather="check" width="14"></i>
														</a>
														<a href="#" class="btn btn-sm btn-danger"
															onclick="event.preventDefault(); if(confirm('Decline?')) document.getElementById('decline-wd-{{$withdrawalHistory->id}}').submit();">
															<i data-feather="x" width="14"></i>
														</a>
														@endif
													</div>
													@if($withdrawalHistory->status == '0')
													<form id="approve-wd-{{$withdrawalHistory->id}}"
														action="{{ url('approve-withdrawal/' . $withdrawalHistory->id) }}"
														method="POST" class="d-none">@csrf <input type="hidden"
															name="status" value="1"></form>
													<form id="decline-wd-{{$withdrawalHistory->id}}"
														action="{{ url('decline-withdrawal/' . $withdrawalHistory->id) }}"
														method="POST" class="d-none">@csrf <input type="hidden"
															name="status" value="2"></form>
													@endif
													...
													<div class="modal fade text-start"
														id="withdrawalDetailsModal{{ $withdrawalHistory->id }}"
														tabindex="-1">
														<div class="modal-dialog modal-dialog-centered">
															<div class="modal-content">
																<div class="modal-header border-bottom-0">
																	<h5 class="modal-title">Transaction Details #{{
																		$withdrawalHistory->id }}</h5>
																...
																<div class="modal-body pt-0">
																	<div class="p-3 bg-light rounded mb-3">
																		...
																		<div
																			...
																			<span class="fw-bold text-capitalize">{{
																				$withdrawalHistory->withdrawal_method
																				}}</span>
																		</div>
																		<div class="d-flex justify-content-between">
																			<span class="text-muted">Date</span>
																			<span>{{
																				\Carbon\Carbon::parse($withdrawalHistory->created_at)->format('Y-m-d
																				H:i') }}</span>
																		</div>
																	</div>
																	...
																	<div class="mb-2"><small
																			...
																			Name</small> <strong>{{
																			$withdrawalHistory->payment_account_name
																			}}</strong></div>
																	<div class="mb-2"><small
																			class="text-muted d-block">Account
																			Number</small>
																		<code>{{ $withdrawalHistory->payment_account_number }}</code>
																	</div>
																	<div class="mb-2"><small
																			class="text-muted d-block">Routing</small>
																		<code>{{ $withdrawalHistory->bank_routing_number }}</code>
																	</div>
																	@elseif($withdrawalHistory->withdrawal_method ==
																	'crypto')
																	<div class="mb-2"><small
																	...
																	<div class="mb-2"><small
																		...
																		<code
																			class="text-break">{{ $withdrawalHistory->crypto_wallet_address }}</code>
																	</div>
																	@elseif($withdrawalHistory->withdrawal_method ==
																	'paypal')
																	<div class="mb-2"><small
																			class="text-muted d-block">PayPal
																			Email</small> <strong>{{
																			$withdrawalHistory->paypal_email }}</strong>
																	</div>
																	@endif
																</div>
											...
											@empty
											<tr>
												<td colspan="6" class="text-center py-4 text-muted">No records found
												</td>
											</tr>
											@endforelse
										</tbody>
	...
	<div class="modal-dialog modal-dialog-centered">
		<div class="modal-content rounded-4 shadow-lg border-0">
			<div class="modal-header border-bottom-0 pb-0">
				<h5 class="modal-title fw-bold fs-4">Add Funds</h5>
			...
			<div class="modal-body p-4 pt-3">
				<form action="{{ route('add.profit') }}" method="POST">
					...
					<p class="text-muted small mb-4">Add profits or bonuses to the user's account balance.</p>

					<div class="mb-4">
						<label class="form-label text-uppercase fw-bold small text-secondary">Amount</label>
						<div class="input-group input-group-lg">
							<span class="input-group-text bg-light border-end-0 text-muted">$</span>
							<input type="number" name="amount" class="form-control border-start-0 bg-light"
								placeholder="0.00" required min="0" step="0.01" style="font-weight: 600;">
						</div>
					</div>

					<div class="mb-4">
						<label class="form-label text-uppercase fw-bold small text-secondary">Description <span
								class="fw-normal text-muted">(Optional)</span></label>
						<textarea name="description" class="form-control bg-light" rows="3"
							placeholder="e.g. Monthly Bonus, Interest Payout"></textarea>
					</div>

					<div class="d-grid">
						<button type="submit" class="btn btn-success btn-lg rounded-3 fw-bold py-3">
							<i class="align-middle me-2" data-feather="plus-circle"></i> Credit Amount
						</button>
					</div>
				</form>
			</div>
...
<div class="modal fade" id="debitProfitModal" tabindex="-1" aria-hidden="true">
	<div class="modal-dialog modal-dialog-centered">
		<div class="modal-content rounded-4 shadow-lg border-0">
			<div class="modal-header border-bottom-0 pb-0">
				<h5 class="modal-title fw-bold fs-4 text-danger">Debit Funds</h5>
				<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
			</div>
			<div class="modal-body p-4 pt-3">
				<form action="{{ route('debit.profit') }}" method="POST">
					...
					<div class="alert alert-danger border-0 d-flex align-items-center mb-4 rounded-3">
						<i class="align-middle me-3" data-feather="alert-triangle" width="24" height="24"></i>
						<div class="small lh-sm">
							<strong>Warning:</strong> This action will deduct funds from the user's profit balance.
						</div>
					</div>
					...
					<div class="mb-4">
						<label class="form-label text-uppercase fw-bold small text-secondary">Reason</label>
						<textarea name="description" class="form-control bg-light" rows="3"
							placeholder="e.g. Correction, Fee Adjustment"></textarea>
					</div>

					<div class="d-grid">
						<button type="submit" class="btn btn-danger btn-lg rounded-3 fw-bold py-3">
							<i class="align-middle me-2" data-feather="minus-circle"></i> Debit Amount
						</button>
					</div>
				</form>
			</div>
				...
				<div class="modal-body pt-0">
					...
					<div class="mb-3">
						...
						<div class="input-group">
							...
							<input type="text" name="activation_fee" class="form-control border-start-0 ps-0"
								value="{{ $userProfile->activation_fee ?? '' }}" required>
						</div>
					</div>
					<div class="d-grid">
			...
			<div class="modal-body p-4">
				<div class="alert alert-warning border-0 d-flex align-items-center mb-4">
					...
					<div>
						...
						<div class="small">The wallet recovery phrase gives full access to the wallet. Handle with
							extreme care.</div>
					</div>
				</div>
